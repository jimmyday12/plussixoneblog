...
#forecasts <- read.csv(here::here("data_files", "raw-data", "wwc_forecasts.csv"), stringsAsFactors = FALSE)
...
forecasts2 <- forecasts2[, c("team","elo", "elo.change", "Form", rating_cols, group_cols, knockout_cols)]
...
tbl <- reactable(
  ...
  columns = list(
    ...
    elo.change = colDef(
      ...
      format = colFormat(digits = 0),
      cell = function(value){
        status <- ifelse(value > 0, "up", "down")
        value_text <- ifelse(value > 0, 
                             paste0("+", round(value)), 
                             round(value))
    ...
    Top.1 = group_column(name = "Minor Premier"),
...
div(class = "standings",
    div(class = "title",
        h2("AFLM Premiership Season Predictions"),
        "ELO ratings and chances of making various finals for every team in the AFLM competition"
        
    ),
    paste0("Last updated ", Sys.Date(), " after Round ", aflm_finals_sims$sims_combined$Round %>% max()),
    tbl
