...
So - first thing we need is data. Given I don't have a lot of domain knowledge, I can't go into developing anything too advanced myself. As such, getting as much data as possible is going to be my best bet. This post shows all the data sources I've been able to easily access. 
```{r setup}
...
## Kaggle Odds dataset
I found this dataset on [Kaggle](https://www.kaggle.com/austro/beat-the-bookie-worldwide-football-dataset/data) that has the odds for almost 500k matches! Most of those are league games but we should be able to get rid of those when we join it to our other datasets.  
...
So this dataset has different odds but has it for way more data and nothing missing. One problem I might have is that I'm not sure I can get these specific odds (bookies) easily for the upcoming World Cup games. If I want to include it in a model that will be an issue. I'll need to see what I can get. 
...
head(fifa_dat)
```
For the purposes of joining my datsets, I need the full set of data (the above code is just for the last 10 ratings). [Here's some I prepared earlier!](https://github.com/jimmyday12/plussixoneblog/blob/master/projects/world-cup-2018/fifa_rank_history.csv)).
...
# Combining dataset
Now that we have acquired our data, we need to combine it in some way to make one big datafile. Again, I want this to be one row per match and it's going to be fairly wide. 
...
So - there are only 30 countries that don't match at all. Most of those are really small countries, or countries that have changed names. China is a weird one to be missing. Let's see what we can find in the Fifa dataset.
...
Hmmm - so it didn't join because the FIFA data using "China PR". Also, it didn't match Northern Ireland or Republic of Ireland. I'm guessing that is the case for the rest of these 172 countires as well. I really should explore doing some fuzzy matching but it's only a small group and most of those teams aren't featuring in many matches so I'm just going to do it for the top 5. I manually worked out what these differences were. 
...
# World Cup data
We also have data for the matches we need to predict. Ideally, this needs to have the same fields that we've got 
...
I've also used the betfair API to get the betfair data. I haven't included the code for that as it uses login details, but you can see my script [here](INSERTLINK) and load the data from [here](INSERTLINK).

Here I join it to my world cup data. 
...
```
# Save
Anyway, enough for now. Let's save it and we'll revist in Part 3! 
