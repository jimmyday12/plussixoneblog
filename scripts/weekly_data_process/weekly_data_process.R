...
if(last(old_results$results$Home.Team) == last(dat$results$Home.Team) &
   last(old_results$results$Away.Team) == last(dat$results$Away.Team) & 
   last(old_results$results$Date) == last(dat$results$Date)) {
  new_data <- FALSE
...
if(new_data) {
  ...
  # summarise
  sim_dat$sim_data_summary  <- calculate_sim_perc(sim_dat$sim_data_all, 
                                                  season, 
                                                  round, sim_num)
  ...
  sim_dat$simCount <- count_sims(sim_dat$sim_data_all, 
                                 sim_dat$sim_data_summary,
                                 sim_num, 
                                 season, 
                                 round, 
  ...
  finals_dat <- do_finals_sims(sim_data_all = sim_dat$sim_data_all, 
  ...
  finals_dat <- combine_finals_sims(final_game = finals_dat$final_game,
